...
public class InvoiceServiceImpl implements InvoiceService {
	...
	public Invoice generateInvoice(String prescription_id, Invoice invoice) {
		...
		Map<String, Integer> prescription_medication = prescription.getMedication_ids();
		...
		List<String> expected_medication_ids_list = new ArrayList<>(prescription_medication.keySet());
		...
		List<String> retrieved_medication_ids_list = retrieved_medications.stream().map(Medication::getMedication_id)
		...
		if (expected_medication_ids_list.size() != retrieved_medication_ids_list.size()) {
			...
			// Creating array with expected_medication_ids_list and removing
			// retrieved_medication_ids_list
			...
			throw new MedicationNotFoundException(
					"Medications with id: " + String.join(", ", missing_medication_ids) + ", not available!!!");
		}

		for (Medication medication : retrieved_medications) {

			Integer expected_medication_quantity = prescription_medication.get(medication.getMedication_id());

			Integer available_medication_quantity = medication.getMedication_quantity();

			if (expected_medication_quantity > available_medication_quantity)
				throw new MedicationNotFoundException(
						"You need " + expected_medication_quantity + " medications of " + medication.getMedication_id()
								+ ", but only " + available_medication_quantity + " medications available!!!");

			total_medication_price += medication.getMedication_price() * expected_medication_quantity;

			medication.setMedication_quantity(available_medication_quantity - expected_medication_quantity);
		}
...
//        return invoice;
	...
	public Map<String, Object> payMedicationCharges(String invoice_id, Invoice invoice) {
		...
		Invoice retrieved_invoice = invoiceRepository.findById(invoice_id)
				.orElseThrow(() -> new InvoiceNotFoundException("Invoice with id: " + invoice_id + ", not found!!!"));
		...
		if (amount_paying > amount_to_be_paid) {
			...
			invoice_details.put("message",
					"Thanks for using our service, Kindly collect your " + (amount_paying - amount_to_be_paid));

			invoice_details.put("invoice", updated_invoice);
			return invoice_details;
		} else if (amount_paying < amount_to_be_paid) {
			throw new InvoiceNotFoundException(
					"You are paying " + amount_paying + ", but it costs " + amount_to_be_paid + " !!!");
		}

//    	return invoice_details;
		Invoice updated_invoice = invoiceRepository.save(retrieved_invoice);
		invoice_details.put("message", "Thank you purchasing with us, collect your medications....");
		invoice_details.put("invoice", updated_invoice);
		return invoice_details;
	}
